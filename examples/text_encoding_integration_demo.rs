...
use crawlrs::utils::crawl_text_integration::{CrawlTextIntegration, ScrapeResponseInput};
use crawlrs::utils::crawl_text_processor::CrawlTextProcessor;
...
pub async fn integrated_handle_scrape_success(
    ...
    // 1. 创建文本处理集成器
    let text_integration = CrawlTextIntegration::new();
    
    // 2. 准备输入数据
    let input = ScrapeResponseInput {
        content: response_content.as_bytes().to_vec(),
        url: url.to_string(),
        content_type: response_content_type.map(|s| s.to_string()),
        status_code: response_status_code,
    };
    
    // 3. 处理响应内容
    let processed_response = text_integration.process_scrape_response(input).await?;
    
    // 4. 验证处理结果
    if processed_response.processing_success {
        println!("文本处理成功，提取内容长度: {}", processed_response.processed_content.len());
        ...
        Ok(ProcessedScrapeResult {
            ...
            detected_encoding: processed_response.detected_encoding,
            processing_time_ms: processed_response.processing_time_ms,
            content_quality_score: processed_response.content_quality_score,
            success: true,
            error: None,
        })
    } else {
        println!("文本处理失败，使用原始内容: {:?}", processed_response.processing_error);
        
        // 处理失败时返回原始内容
        Ok(ProcessedScrapeResult {
            original_content: response_content.to_string(),
            processed_content: response_content.to_string(), // 使用原始内容
            detected_encoding: "UTF-8".to_string(), // 默认编码
            processing_time_ms: processed_response.processing_time_ms,
...
pub async fn simple_text_processing_example() {
    ...
    match process_text_encoding(text_content.as_bytes()).await {
        Ok(result) => {
            println!("文本处理成功:");
            println!("  原始长度: {} 字节", text_content.len());
            println!("  处理后长度: {} 字节", result.text.len());
            println!("  检测到的编码: {}", result.detected_encoding);
    ...
    match process_web_content(html_content.as_bytes(), "https://example.com").await {
        Ok(result) => {
            println!("\nHTML内容处理成功:");
            println!("  提取的文本: {}", result.extracted_text);
            println!("  检测到的语言: {:?}", result.detected_language);
            println!("  内容质量评分: {:.2}", result.content_quality_score);
            println!("  处理时间: {} ms", result.processing_time_ms);
        }
        Err(e) => {
...
pub async fn batch_processing_example() {
    ...
    let inputs = vec![
        CrawlTextInput {
            content: "测试内容1 - 普通文本".as_bytes().to_vec(),
            url: "https://example1.com".to_string(),
            content_type: "text/plain".to_string(),
        },
        CrawlTextInput {
            content: "<html><body><h1>测试内容2</h1><p>HTML内容</p></body></html>".as_bytes().to_vec(),
            url: "https://example2.com".to_string(),
            content_type: "text/html".to_string(),
        },
        CrawlTextInput {
            content: r#"\u8fd9\u662f\u4e00\u4e2a\u0020Unicode\u0020\u6d4b\u8bd5"#.as_bytes().to_vec(),
    ...
    for (i, result) in results.iter().enumerate() {
        match result {
            Ok(processed) => {
                success_count += 1;
                total_content_length += processed.text.len();
                println!("  内容 {} 处理成功: URL={}, 长度={}, 质量评分={:.2}", 
                    i + 1, processed.url, processed.text.len(), processed.content_quality_score);
            }
            Err(e) => {
    ...
    println!("  平均处理时间: {:.2} ms/个", total_time.as_millis() as f64 / results.len() as f64);
}
...
pub async fn performance_optimization_example() {
    ...
    let start_time = std::time::Instant::now();
    let result1 = processor.process_text(short_text.as_bytes());
    ...
    let start_time = std::time::Instant::now();
    let result2 = processor.process_text(long_text.as_bytes());
