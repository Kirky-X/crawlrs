...
- [x] [T038] [P1] 搭建 testcontainers 集成测试基础设施，覆盖 di/* 和 bootstrap/* 模块——**Docker 不可用（WSL2 环境），testcontainers 无法实现**；降级为 mock 方案：测试 di 组件显式构造器（cache_module 0%→81%、database_module 0%→77%、engines_module 0%→93%、search_module 0%→94%、repository_module 0%→38%、service_module 0%→38%）+ bootstrap 薄包装（engines 0%→100%、telemetry 0%→98%、config 68%→86%、infrastructure 0%→54%、services 0%→43%、routes 0%→27%）；总覆盖率 86.09%→87.09%（+1.00%）；**90% 目标在无 Docker 环境下结构性不可达**——di/axum_state.rs 210 行（需完整 AppState 31 字段）+ scrape_worker.rs 1365 行（需 Redis/HTTP）+ bootstrap/routes 344 行（需 AppState）= ~1919 行需集成测试；后续若 Docker 可用，可实现 testcontainers 方案 — file: src/di/, src/bootstrap/, tests/integration/
