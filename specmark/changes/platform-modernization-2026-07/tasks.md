...
- **R-cov-001**（基线测量）：✅ 满足
- **R-cov-002**（覆盖率 ≥90%）：✅ 满足——总覆盖率 90.04%（10649/106919 未覆盖），通过 mock 测试方案达成
- **R-cov-003**（TDD 测试）：✅ 满足

- [x] [T038] [P1] 搭建 testcontainers 集成测试基础设施，覆盖 di/* 和 bootstrap/* 模块——Docker 不可用（WSL2 环境），testcontainers 无法实现；降级为 mock 方案并通过多轮 mock 测试补充最终达成 90% 覆盖率目标：① di 组件显式构造器测试（cache_module 0%→81%、database_module 0%→77%、engines_module 0%→93%、search_module 0%→94%、repository_module 0%→38%、service_module 0%→38%）② bootstrap 薄包装测试（engines 0%→100%、telemetry 0%→98%、config 68%→86%）③ 全项目纯逻辑函数 mock 测试补充（workers/search/utils/infrastructure/domain/application/engines/queue 模块，新增约 550+ 个单元测试）④ 修复 3 个被 #[ignore] 的 scheduler_test（reschedule_retry 路径）⑤ 修复 env_var_security 测试并行竞争（17 个测试加 ENV_MUTEX 锁）⑥ 修复 robots.rs 参数顺序 bug；最终覆盖率 **90.04%**（10649/106919 未覆盖），满足 R-cov-002 规范要求（≥90%） — file: src/di/, src/bootstrap/, tests/unit/queue/scheduler_test.rs, src/infrastructure/security/env_var_security.rs, 全项目
