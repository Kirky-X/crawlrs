...
VALUES 
    ('$TEAM_ID'::uuid, 'Test Team', NOW(), NOW())
...
VALUES 
    (gen_random_uuid(), '$TEST_API_KEY', '$TEAM_ID'::uuid, NOW(), NOW())
...
VALUES 
    (gen_random_uuid(), '$TEAM_ID'::uuid, (SELECT id FROM api_keys WHERE key = '$TEST_API_KEY'), 10000, 'credit', 'Initial test credits', NOW())
