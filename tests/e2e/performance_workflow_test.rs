...
async fn test_performance_single_url_benchmark() {
    ...
    for url in test_urls {
        ...
        let content_length = final_data["result"]["content"].as_str().unwrap_or("").len();
    ...
    for (url, duration, content_length, success) in results {
        println!(
            "URL: {}, Duration: {:?}, Content Length: {}, Success: {}",
...
async fn test_performance_concurrent_scraping() {
    ...
    // Performance assertions
    assert_eq!(
        completed_tasks, concurrent_tasks,
    ...
    println!(
        "Concurrent scraping completed: {} tasks in {:?}",
        concurrent_tasks, total_time
    );
}
...
async fn test_performance_batch_crawl() {
    ...
    assert_eq!(
        status, "completed",
        "Batch crawl should complete successfully"
    );
    assert!(
        results.len() >= urls.len() * 3 / 4,
...
async fn test_performance_error_recovery() {
    ...
    println!(
        "Error recovery performance - Invalid URL: {:?}, SSRF: {:?}, Timeout: {:?}",
