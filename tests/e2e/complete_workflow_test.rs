...
async fn test_complete_scrape_workflow() {
    ...
    if create_response.status_code() != StatusCode::CREATED {
        ...
        panic!("Expected 201 CREATED, got {}", create_response.status_code());
    }
    ...
    while status == "queued" || status == "active" {
        if retries >= MAX_RETRIES {
            println!("Task {} still in status '{}' after {} retries", task_id, status, MAX_RETRIES);
        ...
        if retries > 10 && status == "active" {
            println!("Task {} still active after {} retries, waiting extra time...", task_id, retries);
...
async fn test_batch_scrape_workflow() {
    ...
    assert!(all_completed, "All tasks should complete within expected time");
