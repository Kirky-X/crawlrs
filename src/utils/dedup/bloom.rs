...
//! bloom 说"不存在"是权威的（无假阴性），bloom 说"可能存在"需回落 HashSet/DB 校验。
//!
//! ## 内存模型
//!
//! - Linux：优先 `mmap(MAP_HUGETLB)`（2 MiB 大页），失败 fallback 普通 mmap
//! - 其他 Unix：普通 `mmap(MAP_PRIVATE | MAP_ANONYMOUS)`
//! - 非 Unix（Windows/WASM）：`Vec<u8>` heap 分配
//!
//! 容量约 1 M URLs（默认）→ ~1.2 MB（target 1% 假阳性，实际 < 5%）。
//!
//! ## 线程安全
//!
//! `MmapBloom` 是 `Send + Sync`，但 `insert` 需 `&mut self`（spider 原设计为单线程
//! 插入/查询，匹配 `ListBucket` 使用模式）。多线程共享需外层 `Mutex/RwLock`。
...
/// cause incorrect behavior at the `ListBucket` API level.
...
enum AllocKind {
    ...
    /// Heap fallback via `Vec<u8>` (always available, also non-unix path).
    Heap,
}
