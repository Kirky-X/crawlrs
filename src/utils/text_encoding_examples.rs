...
pub fn example_crawl_text_processing() {
    ...
    // 示例2: 批量处理多个内容
    let batch = vec![
        (r#"<html><body><h1>页面1</h1><p>内容1</p></body></html>"#.as_bytes(), "http://example1.com", Some("text/html")),
        (r#"<html><body><h1>页面2</h1><p>内容2</p></body></html>"#.as_bytes(), "http://example2.com", Some("text/html")),
        (b"Plain text content", "http://example3.com", Some("text/plain")),
    ];
    ...
    for (i, result) in results.iter().enumerate() {
        match result {
            Ok(processed) => {
                info!("  内容 {} 处理成功: URL={}, 文本长度={}", 
                    i + 1, processed.url, processed.processed_content.extracted_text.len());
            }
            Err(e) => {
                error!("  内容 {} 处理失败: {}", i + 1, e);
            }
...
pub async fn example_integration_usage() {
    ...
    let scrape_responses = vec![
        ScrapeResponseInput {
            content: r#"<html><body><h1>测试页面1</h1><p>这是第一个测试页面的内容。</p></body></html>"#.as_bytes().to_vec(),
    ...
    // 批量处理爬虫响应
    let results = integration.process_batch_scrape_responses(scrape_responses).await;
    ...
    info!("集成器状态: 启用={}, 配置={:?}", status.enabled, status.config);
}
...
pub fn example_error_handling() {
    ...
    match processor.process_crawled_content(empty_content, "http://example.com", None) {
        Ok(processed) => {
            info!("空内容处理成功: 提取文本长度={}", processed.processed_content.extracted_text.len());
        }
        Err(e) => {
            error!("空内容处理失败: {}", e);
        }
