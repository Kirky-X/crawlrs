...
// 重新导出主要功能
pub use crate::utils::text_encoding::{TextEncodingProcessor, TextEncodingError, process_text_encoding};
pub use crate::utils::web_content_processor::{WebContentProcessor, WebContentError, ProcessedWebContent, process_web_content};
pub use crate::utils::crawl_text_processor::{CrawlTextProcessor, CrawlProcessingError, ProcessedCrawlContent, process_crawled_content, process_crawled_batch};
pub use crate::utils::crawl_text_integration::{CrawlTextIntegration, ProcessedScrapeResponse, ScrapeResponseInput, create_crawl_text_integration};
...
pub fn quick_process_web_content(content: &[u8], content_type: Option<&str>) -> Result<ProcessedWebContent, WebContentError> {
...
mod tests {
    ...
    fn test_quick_process_functions() {
        ...
        // 测试快速爬虫内容处理
        let crawl_result = quick_process_crawl_content(html.as_bytes(), "http://test.com", Some("text/html"));
        assert!(crawl_result.is_ok());
        let processed_crawl = crawl_result.unwrap();
        assert!(processed_crawl.processed_content.extracted_text.contains("测试HTML"));
    }
