...
impl BingSearchEngine {
    pub fn new() -> Self {
        ...
        let title_regex = regex::Regex::new(r#"<h2[^>]*>(.*?)</h2>"#)
            .expect("Failed to compile title regex");
        let link_regex = regex::Regex::new(r#"<a[^>]*href="([^"]*)"[^>]*>"#)
            .expect("Failed to compile link regex");
        let snippet_regex = regex::Regex::new(r#"<p[^>]*>(.*?)</p>"#)
            .expect("Failed to compile snippet regex");
        let html_clean_regex = regex::Regex::new(r#"<[^>]+>"#)
            .expect("Failed to compile HTML clean regex");
    ...
    pub fn build_bing_url(&self, query: &str, page: u32) -> String {
        ...
        format!("{}?{}", base_url, serde_urlencoded::to_string(&params).unwrap_or_default())
    }
    ...
    pub async fn parse_search_results(&self, html: &str, query: &str) -> Result<Vec<SearchResult>, SearchError> {
        // Validate input
        if html.is_empty() {
            return Err(SearchError::EngineError("Empty HTML response received".to_string()));
        }
        ...
        for result_match in result_matches {
            ...
            let title_html = self.title_regex.captures(result_html)
                .and_then(|cap| cap.get(1))
                .map(|m| m.as_str())
                .unwrap_or_default();
            
            // Extract URL from title HTML
            let url = self.link_regex.captures(title_html)
                .and_then(|cap| cap.get(1))
            ...
            let snippet = self.snippet_regex.captures(result_html)
                .and_then(|cap| cap.get(1))
            ...
            let published_date = snippet.as_ref()
        ...
        if parse_errors > 0 {
            eprintln!("Bing search parsing: {} valid results, {} parse errors", valid_results, parse_errors);
        }
        
        // Return error if too many parsing failures
        if valid_results == 0 && parse_errors > 0 {
            return Err(SearchError::EngineError("Failed to parse any search results from HTML response".to_string()));
        }
...
impl SearchEngine for BingSearchEngine {
    ...
    async fn search(&self, query: &str, limit: u32, lang: Option<&str>, country: Option<&str>) -> Result<Vec<SearchResult>, SearchError> {
        ...
        if query.trim().is_empty() {
            return Err(SearchError::EngineError("Search query cannot be empty".to_string()));
        }
        ...
        if limit > 100 {
            return Err(SearchError::EngineError("Search limit cannot exceed 100".to_string()));
        }
        ...
        if lang.len() != 2 {
            return Err(SearchError::EngineError(format!("Invalid language code: {}", lang)));
        }
        
        if country.len() != 2 {
            return Err(SearchError::EngineError(format!("Invalid country code: {}", country)));
        }
        
        // Calculate pages needed based on limit (10 results per page)
        let pages_needed = ((limit + 9) / 10).max(1);
        if pages_needed > 10 {
            return Err(SearchError::EngineError("Too many pages requested".to_string()));
        }
                ...
                match result {
                    Ok((html, page, query_str_result)) => {
                        ...
                        match self.parse_search_results(&html, &query_str_result).await {
                            Ok(page_results) => {
                                ...
                                eprintln!("Bing page {}: {} results, total: {}", page, results_count, all_results.len());
                            }
                            Err(e) => {
                    ...
                    Err(e) => {
                        ...
                        if consecutive_errors >= MAX_CONSECUTIVE_ERRORS {
                            return Err(SearchError::NetworkError(format!("Too many consecutive errors: {}", e)));
                        }
                    }
                }
        ...
        if all_results.is_empty() {
            return Err(SearchError::EngineError("No search results found".to_string()));
        }
