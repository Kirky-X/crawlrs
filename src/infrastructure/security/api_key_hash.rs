...
//! API Key 哈希与验证模块
//!
//! 提供 API Key 的安全哈希存储与验证功能：
//! - [`hash_api_key`]：使用 bcrypt（cost = `DEFAULT_COST`）进行密码学安全哈希
//! - [`verify_api_key`]：验证 API Key 是否匹配 bcrypt 哈希
//! - [`hash_api_key_sha256`]：SHA-256 摘要，用于缓存键（**非**密码存储）
//! - [`is_legacy_sha256_hash`]：检测旧版 SHA-256 哈希格式以便迁移
//!
//! # 设计原则
//!
//! - bcrypt 哈希用于存储 API Key（带随机盐，抵抗彩虹表与暴力破解）
//! - SHA-256 仅用于缓存键等非密码学场景，**禁止**用于密码存储
//! - 旧版 SHA-256 哈希通过 [`is_legacy_sha256_hash`] 识别，验证成功后
//!   应由调用方触发迁移到 bcrypt
//!
//! # 安全说明
//!
//! - bcrypt `DEFAULT_COST`（12）在 2025 年硬件下提供合理的暴力破解防护
//! - `verify_api_key` 使用 `unwrap_or(false)` — 验证失败与解析失败均返回
//!   `false`，避免泄露哈希格式信息
...
/// 计算 API Key 的 bcrypt 哈希值
///
/// 使用 bcrypt 对 API Key 进行哈希处理，提供强大的暴力破解防护。
/// 每次调用产生不同的哈希值（随机盐），需通过 [`verify_api_key`] 验证。
///
/// # 参数
///
...
/// # 返回值
///
/// * `Ok(String)` - bcrypt 哈希字符串（以 `$2b$` 开头）
/// * `Err` - bcrypt 哈希失败（如 API Key 过长）
///
/// # 示例
///
/// ```ignore
...
/// 验证 API Key 是否匹配 bcrypt 哈希值
///
/// # 参数
///
/// * `api_key` - 原始 API Key 字符串
/// * `key_hash` - bcrypt 哈希字符串（由 [`hash_api_key`] 产生）
///
/// # 返回值
///
/// * `true` - 验证成功
...
/// # 安全说明
///
/// 验证失败与哈希格式错误均返回 `false`，不向调用方区分失败原因，
/// 避免泄露哈希格式信息。
...
///
/// 旧版格式为纯十六进制字符串（64 字符），新版 bcrypt 哈希以 `$2b$` 开头。
/// 调用方检测到旧版哈希后应在验证成功时触发迁移到 bcrypt。
///
/// # 参数
///
/// * `key_hash` - 要检测的哈希值
...
/// # 返回值
///
/// * `true` - 是旧版 SHA-256 格式（64 字符纯十六进制）
/// * `false` - 是新版 bcrypt 格式（以 `$` 开头）
...
/// # 安全警告
///
/// **这不是 bcrypt，不能用于密码存储。** SHA-256 是快速哈希，对暴力破解
/// 无防护能力。仅用于需要确定性、固定长度摘要的非密码学场景。
///
/// # 参数
///
/// * `api_key` - 原始 API Key 字符串
