...
// teams-off：移除 GR 泛型 + geo_restriction_repo + team_service 参数 + 地理限制块
//   （单租户降级，无地理限制概念，直接建 Task 入队）
//
// 路由装配（bootstrap::routes）：
//   teams-on: `post(extract_handler::extract::<DatabaseGeoRestrictionRepository>)`
//   teams-off: `post(extract_handler::extract)`
#[cfg(feature = "teams")]
...
/// （单租户降级，无地理限制概念，直接建 Task 入队）。
///
/// 保留：SSRF 防护、sync_wait_ms 验证、Task 构造、队列入队、同步等待逻辑
/// 移除：`Extension<Arc<GR>>`、`Extension<Arc<TeamService>>`、`ConnectInfo<SocketAddr>` 改为 `_addr`
///   （ConnectInfo 保留是因为 axum 路由装配仍需要，但 IP 在单租户模式下不用于地理限制）
