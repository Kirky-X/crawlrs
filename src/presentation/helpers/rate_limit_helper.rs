...
/// # Arguments
///
/// * `service` - The rate limiting service
...
pub async fn check_rate_limit<T: RateLimitingService>(
    service: &Arc<T>,
    api_key: &str,
    endpoint: &str,
...
mod tests {
    ...
    async fn test_allowed_passed() {
        ...
        let result = check_rate_limit(&Arc::new(service), "test-key", "/v1/test").await;
        assert!(result.is_ok());
