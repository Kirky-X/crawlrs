...
mod tests {
    ...
    async fn test_worker_manager_configuration_loading() {
        ...
        assert!(settings.database.url.len() > 0, "Database URL must be configured");
        ...
        println!("  Database URL configured: {}", if settings.database.url.is_empty() { "[EMPTY]" } else { "[SET]" });
        println!("  Redis URL configured: {}", if settings.redis.url.is_empty() { "[EMPTY]" } else { "[SET]" });
        println!("  Default team limit: {}", settings.concurrency.default_team_limit);
        println!("  Task lock duration: {} seconds", settings.concurrency.task_lock_duration_seconds);
    ...
    fn test_worker_settings_structure() {
        ...
        println!("  Default team limit: {}", concurrency_settings.default_team_limit);
        println!("  Task lock duration: {}s", concurrency_settings.task_lock_duration_seconds);
        
        // Verify reasonable defaults
        assert!(concurrency_settings.default_team_limit >= 1, "Minimum team limit should be 1");
        assert!(concurrency_settings.task_lock_duration_seconds >= 1, "Minimum task lock duration should be 1 second");
    }
}