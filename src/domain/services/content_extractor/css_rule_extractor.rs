...
//! 5. 页面类型：默认 `PageType::Other`（CSS 启发式不足以分类）
//!
...
impl ContentExtractor for CssRuleExtractor {
    fn extract(&self, html: &str, _url: Option<&str>) -> Result<ExtractedContent, ContentExtractionError> {
...
mod tests {
    ...
    fn extracts_main_content_from_simple_article() {
        ...
        let result = extractor.extract(html, None).expect("extract ok");
    ...
    fn extracts_title_from_title_tag() {
        ...
        let result = CssRuleExtractor::new().extract(html, None).expect("extract ok");
    ...
    /// 空的 author content 应被过滤为 None
    #[test]
    fn returns_none_author_when_meta_content_empty() {
        let html = r#"<html><head>
            <meta name="author" content="   ">
            </head><body><p>content</p></body></html>"#;
        let result = CssRuleExtractor::new().extract(html, None).expect("extract ok");
        assert!(result.author.is_none(), "empty author should be None");
    }

    /// 空 HTML 应返回 EmptyContent 错误
    #[test]
    fn returns_error_for_empty_html() {
        let result = CssRuleExtractor::new().extract("", None);
    ...
    fn confidence_is_fixed_at_0_5() {
        ...
        assert!(result.should_fallback_to_llm(), "0.5 should trigger fallback");
    }

    /// page_type 应为 Other
    #[test]
    fn page_type_is_other() {
        let html = "<html><body><p>some content</p></body></html>";
        let result = CssRuleExtractor::new().extract(html, None).expect("extract ok");
