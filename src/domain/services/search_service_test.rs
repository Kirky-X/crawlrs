...
mod tests {
    ...
    fn test_search_service_configuration_loading() {
        ...
        println!("  Google API Key configured: {}", if settings.google_search.api_key.as_ref().unwrap().is_empty() { "[EMPTY]" } else { "[SET]" });
        println!("  Google CX configured: {}", if settings.google_search.cx.as_ref().unwrap().is_empty() { "[EMPTY]" } else { "[SET]" });
        ...
        if settings.google_search.api_key.as_ref().unwrap().is_empty() || settings.google_search.cx.as_ref().unwrap().is_empty() {
